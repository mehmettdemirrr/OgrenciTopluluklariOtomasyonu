...
- `audit.read` izni (yalnız Admin, claim Id=36) tek satırlık migration ile eklendi (`20260822..._Faz13_DenetimIziVeReferansVeriOlgunlugu`). `IAuditLogDal`/`EfAuditLogDal` (DataAccess) + `IAuditLogService`/`AuditLogManager` (Business, aspect'siz — filtreleri olduğu gibi DAL'a iletir) + `GET /api/audit-logs` (`entityType`/`entityId`/`userId`/`fromUtc`/`toUtc` + sayfalama). `AuditLog` `IEntity` implemente etmediği için (bilinçli — kendi kendini audit'lememesi) generic repository'den değil, `IReportDal` precedent'iyle özel bir DAL'dan okunuyor.
- Referans verisi olgunlaştı: `PUT /api/faculties/{id}`, `PUT /api/academic-terms/{id}`, `PUT|DELETE /api/faculties/{id}/departments/{deptId}` — plandaki bulleted liste birebir uygulandı (fakülte/dönem yalnızca PUT, yalnızca bölüm hard delete edilebiliyor).
- **A-12'nin hard-delete kararı yeni bir katman-nötr çeviri gerektirdi:** `Core.DataAccess.ReferentialIntegrityConflictException` eklendi ve `AppDbContext.SaveChangesAsync` bunu **yalnızca gerçek FK ihlalinde** (`SqlException.Number == 547`) fırlatıyor — ilk yazımda tüm `DbUpdateException`'ları yakalayan daha geniş bir `catch` denendi, ancak bu `DomainConstraintTests`'in iki mevcut testini kırdı (unique index ihlallerinde ham `DbUpdateException` bekliyorlardı — A-15'in doğrudan DB sözleşmesi). Canlı test çalıştırmasında yakalanıp düzeltildi; artık yalnızca FK Restrict ihlalleri (547) çevriliyor, unique index ihlalleri eskisi gibi ham EF tipiyle bubbling yapıyor (yalnızca doğrudan `AppDbContext` kullanan entegrasyon testlerinde görünür, Business katmanına zaten hiç sızmıyordu).
- `ReportType.TermSummary` eklendi — `ReportManager.RequestAsync`/`DownloadAsync` ve `ReportGenerationManager.GenerateAsync` üçlü switch'e çevrildi; TermSummary'nin `ClubMembers`/`EventParticipants`'tan farkı: tekil bir hedef kulüp yok, kapsam kontrolü `scope.AllClubs || scope.ClubIds.Count > 0` ile yapılıyor. **`DownloadAsync`'in Y-51 (3/3) kontrolü ilk yazımda unutulmuştu** — canlı entegrasyon testinde (`TermSummaryReportTests`) indirme adımı 403 döndü, kaynak `ResolveClubIdForScopeCheckAsync`'in TermSummary'de her zaman `null` dönmesiydi; düzeltilip teste bağlandı. `ExcelReportBuilder.BuildTermSummaryWorkbook` mevcut `ClosedXML` kalıbını birebir izliyor.
- Frontend: `AuditLogPage` (`/audit`, filtre + `DataTable`), `ReferenceDataPage`'e düzenle (kalem ikonu) + sil (çöp kutusu + `ConfirmDialog`) aksiyonları, `ReportsPage`'in rapor türü seçici artık gerçek bir `Select` (TermSummary seçilince kulüp seçici gizleniyor). `SideNav`'a "Denetim İzi" linki eklendi.
- Testler: `ReferenceDataManagerTests`'e 6 (Update×2 kabul/ret + Delete kabul/ret), `AcademicTermManagerTests`'e 2, yeni `AuditLogManagerTests` 2, `ReportManagerTests`/`ReportGenerationManagerTests`'e TermSummary için 5 yeni birim testi; entegrasyon tarafında `ReferenceDataEndpointTests`'e 5 (PUT×3, DELETE kabul+ret/409), yeni `AuditLogEndpointTests` (Y-26 sızıntı yok + `audit.read` olmayan 403), yeni `TermSummaryReportTests` (kuyruk→xlsx indirme uçtan uca). Toplam 251/251 test yeşil (169 Business.Tests + 10 Architecture.Tests + 72 WebAPI.IntegrationTests).
- Canlı doğrulama: admin ile `/audit` sayfası filtrelerle birlikte açıldı (PasswordHash vb. hiç görünmüyor), `/reference`'ta yeni fakülte oluşturulup adı değiştirildi, yeni bölüm oluşturulup silindi, `/reports`'ta Dönem Özeti talep edilip "Kuyrukta" durumuyla listeye düştüğü teyit edildi (kuyruktan geçip indirilebildiği zaten `TermSummaryReportTests` ile uçtan uca kanıtlı). Konsol/HTTP hatası sıfır.
...
- A-42 birebir uygulandı: tek rota öneki (`/api/public/*`), tek controller (`PublicContentController`, controller seviyesinde `[AllowAnonymous]` — dosyanın tamamı anonim, `git diff` ile denetlenebilir), tek servis (`IPublicContentService`/`PublicContentManager`, `[SecuredOperation]` yok), ayrı DTO ailesi (`Business/DTOs/Public/` — `PublicClubListItemDto`, `PublicClubDetailDto`, `PublicEventListItemDto`, `PublicAnnouncementListItemDto`; mevcut yetkili DTO'lar hiç import edilmedi). Filtreler kodda sabit: kulüp `IsActive`, etkinlik `Status==Published && StartDateUtc>=now`, duyuru `Visibility==Public` — `EventManager.GetUpcomingAsync`/`ClubManager.GetListPagedAsync`'in filtreleriyle aynı mantık, ayrı kod yolunda.
- `GET /api/public/events` ve `GET /api/public/announcements` plandaki dört ucun (kulüpler, kulüp detay, etkinlikler, duyurular) üçünü tek `clubId` opsiyonel sorgu parametresiyle birleştirdi — `/kulupler/:id` sayfası "o kulübün açık duyuru/etkinlikleri" ihtiyacını ayrı bir beşinci uç açmadan bu parametreyle karşılıyor.
- **14.4'ün cache kararı yeni bir altyapı değişikliği gerektirdi:** bir yazma ucunun (ör. `ClubManager.SetStatusAsync`) hem kendi (`"ClubManager."`) hem `"PublicContentManager."` önbelleğini düşürmesi gerekiyordu, ama `CacheRemoveAspectAttribute` `AllowMultiple=false` olduğu için aynı metoda iki kez eklenemiyordu. Attribute `params string[] patterns` alacak şekilde genişletildi (`CacheRemoveAspectHandler` artık desenler üzerinde dönüyor) — geriye dönük uyumlu, mevcut tüm tek-desenli kullanımlar değişmeden çalışıyor. `CacheKeyPatternContractTests` (Faz 7'nin kapanış testi) `.Patterns.Single()`'a güncellendi. `[CacheRemoveAspect("PublicContentManager.")]` eklenen uçlar: `ClubManager.CreateAsync/UpdateAsync/SetStatusAsync`, `EventManager.DecideAsync`, `AnnouncementManager.CreateAsync/UpdateAsync/DeleteAsync/CreateGlobalAsync`. Üç okuma ucu (`GetClubsAsync`, `GetEventsAsync`, `GetAnnouncementsAsync`) `[CacheAspect(10)]` taşıyor; `GetClubByIdAsync` `ClubManager.GetByIdAsync` precedent'iyle aynı gerekçeyle cache'siz kaldı.
- Frontend: `PublicLayout` (Faz 8'de iskelet olarak bırakılmıştı) dolduruldu — kimliği doğrulanmışsa "Panele Git", değilse "Giriş Yap"/"Kayıt Ol". `HomePage` (`/`, hero + duyuru şeridi + yaklaşan etkinlik kartları + kulüp vitrini), `PublicClubsPage` (`/kulupler`, arama), `PublicClubDetailPage` (`/kulupler/:id`), `PublicEventsPage` (`/etkinlikler`). Wildcard rota `/panel`'den `/`'e çevrildi. `apiClient`'ın 401-refresh interceptor'ı `/public/` içeren istekleri muaf tutacak şekilde güncellendi (planın öngördüğü en somut regresyon riski, canlı doğrulamada `/auth/refresh`'in hiç tetiklenmediği teyit edildi).
- Testler: `PublicContentManagerTests` (6 — gerçek predicate'in LINQ-to-Objects üzerinde çalıştırıldığı, salt-geçiş olmayan testler: aktif/pasif kulüp ayrımı, Draft/PendingApproval/Rejected/geçmiş etkinliklerin elenmesi, Members duyurusunun elenmesi, `clubId` filtresi), entegrasyon tarafında planın kendi adlandırdığı **`PublicSurfaceLeakTests`** (3 test — token hiç gönderilmeden üç uç 200 dönüyor, seed edilen danışman/öğrenci e-postası ve öğrenci numarası hiçbir cevapta geçmiyor, Draft/PendingApproval etkinlik ve Members duyurusu sızmıyor) ve **`PublicEndpointAuthorizationTests`** (11 — sekiz farklı korumalı uç token'sız 401 dönüyor, üç `/api/public/*` ucu token'sız 200 dönüyor). Toplam 271/271 test yeşil (175 Business.Tests + 10 Architecture.Tests + 86 WebAPI.IntegrationTests).
- Canlı doğrulama: danışman hesabıyla gerçek bir yayında etkinlik + herkese açık duyuru oluşturulup, hiç giriş yapılmadan `/` açıldığında hero/duyuru/etkinlik/kulüp bölümlerinin hepsinin doğru içerikle göründüğü, `/kulupler` → `/kulupler/:id` → `/etkinlikler` gezintisinin sorunsuz çalıştığı, oturum açık bir kullanıcı SPA içi gezinmeyle `/`'a döndüğünde üst barda "Panele Git" çıktığı (ilk denemede `page.goto` ile tam sayfa yenilemesi kullanıldığı için bu senaryo yanlış negatif verdi — script SPA içi gezinmeye çevrilip düzeltildi, uygulama kodunda hata yoktu) tarayıcıda teyit edildi. Konsol hatası, beklenmeyen `/auth/refresh` çağrısı ve HTTP hatası sıfır.

...
export const theme = createTheme({
  palette: {
    ...
    divider: alpha(brand.grey, 0.35),
    text: {
      primary: brand.navy,
      secondary: brand.greyDark,
    },
    background: {
      default: '#F4F6FA',
      paper: '#ffffff',
  ...
  components: {
    ...
    MuiPaper: {
      styleOverrides: {
        ...
        outlined: {
          borderColor: alpha(brand.grey, 0.32),
        },
      },
      defaultProps: {
        elevation: 0,
    ...
    MuiCard: {
      styleOverrides: {
        root: {
          borderRadius: 16,
          overflow: 'hidden',
          border: `1px solid ${alpha(brand.grey, 0.32)}`,
          boxShadow: `0 1px 2px ${alpha(brand.navy, 0.04)}`,
          transition: 'transform 180ms ease, box-shadow 180ms ease, border-color 180ms ease',
          '&:hover': {
            transform: 'translateY(-3px)',
            borderColor: alpha(brand.turquoise, 0.45),
            boxShadow: `0 14px 32px ${alpha(brand.navy, 0.1)}`,
          },
        },
      },
    },
    ...
    MuiMenu: {
      styleOverrides: {
        paper: {
          borderRadius: 12,
          border: `1px solid ${alpha(brand.grey, 0.28)}`,
          boxShadow: `0 16px 40px ${alpha(brand.navy, 0.14)}`,
        },
      },
    },
    MuiPaginationItem: {

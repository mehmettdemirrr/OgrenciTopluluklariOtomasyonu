...
public class AuditLogManagerTests
{
    ...
    {
        _auditLogDal.Setup(d => d.GetPagedAsync("Club", "5", 7, null, null, null, 0, 20, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PagedResult<AuditLogListItemDto>([], 0, 0, 20));

        var result = await _sut.GetPagedAsync("Club", "5", 7, null, null, null, 0, 20);

        Assert.True(result.IsSuccess);
        _auditLogDal.Verify(d => d.GetPagedAsync("Club", "5", 7, null, null, null, 0, 20, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    {
        _auditLogDal.Setup(d => d.GetPagedAsync(null, null, null, null, null, null, 0, 20, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PagedResult<AuditLogListItemDto>([], 0, 0, 20));

        var result = await _sut.GetPagedAsync(null, null, null, null, null, null, 0, 0);

        Assert.True(result.IsSuccess);
        _auditLogDal.Verify(d => d.GetPagedAsync(null, null, null, null, null, null, 0, 20, It.IsAny<CancellationToken>()), Times.Once);
    }
}
